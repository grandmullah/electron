  ...
  const printThermalTicket = (bet: AgentBet) => {
    ...
    const thermalHTML = `
      ...
      <html>
        ...
        <body>
          <div class="header">
            <div class="logo">🎯 BETZONE</div>
            <div>BET TICKET</div>
            <div class="bet-id">ID: ${bet.id.substring(0, 16)}...</div>
          ...
          <div class="section">
            ...
            <div class="row">
              ...
              <span class="row-value">${bet.userPhone}</span>
            </div>
            <div class="row">
              <span class="row-label">Country:</span>
              <span class="row-value">${bet.userCountry}</span>
            </div>
          </div>
          
          <div class="section">
            <div class="section-title">BET SUMMARY</div>
            <div class="row">
              <span class="row-label">Type:</span>
              <span class="row-value status">${bet.betType.toUpperCase()}</span>
            </div>
            <div class="row">
              <span class="row-label">Status:</span>
              <span class="row-value status">${bet.status.toUpperCase()}</span>
            </div>
            <div class="row">
              <span class="row-label">Stake:</span>
              <span class="row-value amount">$${bet.totalStake.toFixed(
                2
              )}</span>
            </div>
            <div class="row">
              <span class="row-label">Potential:</span>
              <span class="row-value amount">$${bet.potentialWinnings.toFixed(
                2
              )}</span>
            </div>
            ${
              bet.betType === "multibet"
                ? `
            <div class="row">
              <span class="row-label">Combined Odds:</span>
          ...
          <div class="section">
            ...
            ${bet.selections
              .map(
                (selection, index) => `
              <div class="selection">
                <div class="selection-header">SELECTION ${index + 1}</div>
                <div class="teams">${selection.homeTeam} vs ${
                  selection.awayTeam
                }</div>
                <div class="bet-details">
                  <div>${selection.betType}: ${selection.selection}</div>
                  <div>Odds: ${selection.odds} | Game: ${selection.gameId}</div>
                  ${
                    selection.stake
                      ? `<div>Stake: $${selection.stake.toFixed(2)} → $${
                          selection.potentialWinnings?.toFixed(2) || "0.00"
                        }</div>`
                      : ""
                  }
                </div>
          ...
          <div class="barcode">
            ${bet.id.substring(0, 12).toUpperCase()}
          </div>
          
          <div class="footer">
            <div>OFFICIAL BET TICKET</div>
            <div>Keep this ticket for your records</div>
            <div>Agent ID: ${bet.agentId.substring(0, 8)}...</div>
            <div>www.betzone.app</div>
          </div>
